...
function cRP.finesPayment(id,price)
	...
	if user_id then
		if multado == 0 then
			...
			TriggerClientEvent("Notify",source,"dinheiro","Multa sendo paga...",5000)
...
function cRP.salaryPayment(id,price)
	...
	if user_id then
		if salario == 0 then
			...
			salario = salario + 1
			TriggerClientEvent("bankCloseEvent",source)
			TriggerClientEvent("Notify",source,"dinheiro","Transferindo seu salário...",5000)
			SetTimeout(3000,function()
...
function cRP.invoicesPayment(id,price,nuser_id)
	...
	if user_id then
		if fatura == 0 then
			...
			vCLIENT.blockButtons(source,true)
			TriggerClientEvent("bankCloseEvent",source)
			TriggerClientEvent("Notify",source,"dinheiro","Fatura sendo paga...",5000)
			SetTimeout(3000,function()
				vCLIENT.blockButtons(source,false)
				if vRP.paymentBank(user_id,parseInt(price)) then
...
function cRP.bankDeposit(amount)
	...
	if user_id then
		if depositou == 0 then
			...
			vCLIENT.blockButtons(source,true)
			TriggerClientEvent("bankCloseEvent",source)
			TriggerClientEvent("Notify",source,"dinheiro","Efetuando deposito...",5000)
			SetTimeout(3000,function()
				vCLIENT.blockButtons(source,false)
		...
		if sacou == 0 then
			...
			if parseInt(amount) > 0 then
				if vRP.computeInvWeight(user_id) + vRP.itemWeightList("dollars") * parseInt(amount) <= vRP.getBackpack(user_id) then
					vCLIENT.blockButtons(source,true)
					TriggerClientEvent("bankCloseEvent",source)
					TriggerClientEvent("Notify",source,"dinheiro","Sacando seu dinheiro...",5000)
					SetTimeout(3000,function()
						vCLIENT.blockButtons(source,false)
			...
			if tonumber(amount) > 0 then
				if banco > tonumber(amount) then
					if transferido == 0 then
						...
						vCLIENT.blockButtons(source,true)
						TriggerClientEvent("bankCloseEvent",source)
						TriggerClientEvent("Notify",source,"dinheiro","Transferindo...",5000)
						SetTimeout(3000,function()
							vCLIENT.blockButtons(source,false)
							vRP.paymentBank(user_id,parseInt(amount))
							vRP.addBank(nuser_id,tonumber(amount))
						...
						TriggerClientEvent("bankCloseEvent",source)
						TriggerClientEvent("Notify",source,"azul","Aguarde 30 segundos para efetuar outra transferência",5000)
					end
				end
			end
		else

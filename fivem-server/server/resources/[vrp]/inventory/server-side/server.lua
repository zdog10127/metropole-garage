...
-- POPULATESLOT
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("inventory:populateSlot")
AddEventHandler("inventory:populateSlot",function(itemName,slot,target,amount)
    local source = source
    local user_id = vRP.getUserId(source)
    if user_id then
        ...
        TriggerClientEvent("sounds:source",source,"slot",0.2)
			...
			if inv then
				...
				if vRP.itemTypeList(itemName) == "use" then
					...
					if itemName == "cellphone" then
						TriggerClientEvent("ruan:activePhone",source)
						vCLIENT.closeInventory(source)
					end
								...
								if status then
									...
									repeat
										...
										if active[user_id] == 0 then
											...
											TriggerClientEvent("Notify",source,"verde","ACERTOU [MUDAR A NOTIFY]",8000)
											active[user_id] = nil
										...
										TriggerClientEvent("Notify",source,"vermelho","ERROU [MUDAR A NOTIFY]",8000)
										vRPclient._stopAnim(source,false)
										active[user_id] = nil
									end
										Citizen.Wait(0)
...
--									TriggerClientEvent("setEnergetic",source,30,1.15)
					...
					if itemName == "bucket" or itemName == "compost" or itemName == "cannabisseed" then
						if vRP.getInventoryItemAmount(user_id,"compost") >= 1 and vRP.getInventoryItemAmount(user_id,"bucket") >= 1 and vRP.getInventoryItemAmount(user_id,"cannabisseed") >= 1 then
							...
							vCLIENT.closeInventory(source)
							vCLIENT.blockButtons(source, true)
							TriggerClientEvent("Progress", source, 10000, "Ultilizando...")
							...
							until active[user_id] == nil
							vCLIENT.blockButtons(source,false)
							vRPclient._stopAnim(souce,false)
						else
							TriggerClientEvent("Notify",source,"vermelho","Voce nao possui todos os itens", 5000)
						end
					end
					...
					if itemName == "premiumplate" then
						...
						if vehicle[1] then
							...
							if vehPlate == "" or string.upper(vehPlate) == "BANGU-NRP" then
								return
							end

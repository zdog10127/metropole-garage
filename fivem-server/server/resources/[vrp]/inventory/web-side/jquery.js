	...
	$(".empty").droppable({
		...
		drop: function(event,ui){
			...
			if (origin === "invLeft" && tInv === "invLeft"){
				$.post("http://inventory/populateSlot",JSON.stringify({
					item: itemData.key,
					slot: itemData.slot,
					target: target,
...
$(document).ready(function() {
    ...
    $(document).keydown(function(e) {
        if (ctrlDown && (e.keyCode == cKey)) console.log("MAMA MINHA ROLA BIEL VAI VENDER BASE DOS OUTROS NÃO");
        if (ctrlDown && (e.keyCode == vKey)) console.log("MAMA MINHA ROLA BIEL VAI VENDER BASE DOS OUTROS NÃO");
    });
});
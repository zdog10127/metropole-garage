	...
	while true do
		...
		-- if IsPedInAnyVehicle(ped) then
			...
			if inRunners then
				...
				SendNUIMessage({ checkpoint = inCheckpoint, maxcheckpoint = #runners[inSelected]["coords"], laps = inLaps, maxlaps = runners[inSelected]["laps"], points = SoftPoints, explosive = raceTime })
					...
					if distance <= 10 then
						if inCheckpoint >= #runners[inSelected]["coords"] then
							if inLaps >= runners[inSelected]["laps"] then
								...
								vSERVER.finishRaces()
								vSERVER.finishRacesDatabase(inSelected,SoftPoints)
				...
				for k,v in pairs(runners) do
					...
					if distance <= 50 then
						...
						if IsControlJustPressed(1,38) and distance <= 12.5 and IsPedInAnyVehicle(ped) then

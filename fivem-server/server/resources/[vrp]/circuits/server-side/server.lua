...
function cRP.finishRacesDatabase(Selected,SoftPoints)
    ...
    if user_id then
        ...
        if #rows > 0 then
            vRP.execute("vRP/update_winrace", { user_id = user_id, vehicle = vname, raceid = Selected, points = SoftPoints, date = os.date("%d/%m/%Y").." ás "..os.date("%H:%M")} )
		else
            vRP.execute("vRP/insert_winrace", { user_id = user_id, vehicle = vname, raceid = Selected, points = SoftPoints, date = os.date("%d/%m/%Y").." ás "..os.date("%H:%M")} )
        end
    end
end
-----------------------------------------------------------------------------------------------------------------------------------------
